              ...
              (term expression
                    (lambda (a b)
                      ...
                      (cond ((and (atom  a) (listp b)) (append (list a) b))
                            ...
                            ((and (listp a) (listp b)
                                  (not (listp (car b)))
                                  (not (alexandria:length= 1 (symbol-name (car b)))))
                             (list a b))
                            (t (cons a b))))))
  (term   :null
          (:variable (lambda (x)
...
(defun fx-pat (pat &optional (eval t))
  (let* (;; given that we are going to get a cycle anyway
    ...
    (if eval
        (if (alexandria:length= 1 cm-patterns) (first cm-patterns) cm-patterns)
        (if (alexandria:length= 1 lisp-patterns) (first lisp-patterns) lisp-patterns))))
