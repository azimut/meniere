...
(define-vug square (freq mfreq amp)
...
(dsp! dsp-square (freq amp dur atk)
  (:defaults 110 .1 1 .25)
  (with-samples ((in (sine freq amp))
                 (in (square in freq amp))
...
(dsp! dsp-pulse (freq amp dur atk cutoff-freq)
  (:defaults 440 .1 1 .25 1000)
  (with-samples ((i (pulse freq amp))
                 (i (lpf i cutoff-freq 1))
                 (i (* i (envelope (make-perc atk dur)
                                   1 dur #'incudine:free))))
    (stereo i)))
